...
async function run() {
  ...
  for (const row of drivers) {
    ...
    if (fn !== row.name || fc !== row.cpf || fp !== row.phone) {
      await supabase.from('drivers').update({ name: fn, cpf: fc, phone: fp }).eq('id', row.id);
      dUpdates++;
    }
  }
  console.log(`Updated ${dUpdates}/${drivers.length} drivers`);
  ...
  for (const row of owners) {
    ...
    if (fn !== row.name || fc !== row.cpf_cnpj || fp !== row.phone) {
      await supabase.from('owners').update({ name: fn, cpf_cnpj: fc, phone: fp }).eq('id', row.id);
      oUpdates++;
    }
  }
  console.log(`Updated ${oUpdates}/${owners.length} owners`);
  ...
  for (const row of clients) {
    ...
    if (fnR !== row.razao_social || fnF !== row.nome_fantasia || fCnpj !== row.cnpj || fp !== row.phone || fCity !== row.city || fState !== row.state) {
      await supabase.from('clients').update({ 
        razao_social: fnR, nome_fantasia: fnF, cnpj: fCnpj, phone: fp, city: fCity, state: fState 
      }).eq('id', row.id);
      cUpdates++;
    }
  }
  console.log(`Updated ${cUpdates}/${clients.length} clients`);
  ...
  for (const row of cargos) {
    ...
    if (fo !== row.origin || fd !== row.destination) {
      await supabase.from('cargos').update({ origin: fo, destination: fd }).eq('id', row.id);
      cgUpdates++;
    }
  }
  console.log(`Updated ${cgUpdates}/${cargos.length} cargos`);
  ...
  for (const row of shipments) {
    ...
    if (fn !== row.driver_name || fc !== row.driver_cpf || fp !== row.driver_contact || fop !== row.owner_contact) {
      await supabase.from('shipments').update({ 
        driver_name: fn, driver_cpf: fc, driver_contact: fp, owner_contact: fop 
      }).eq('id', row.id);
      sUpdates++;
    }
  }
  console.log(`Updated ${sUpdates}/${shipments.length} shipments`);
